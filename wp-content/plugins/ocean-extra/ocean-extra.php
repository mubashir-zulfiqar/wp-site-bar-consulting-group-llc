...
function oceanwp_addons_fs(){
    ...
    foreach ($oceanwp_addons as $addon) {
        $fs_name = str_replace( '-', '_', $addon . '_fs' );
        global ${$fs_name};
        //echo $fs_name . "<br>";
        ${$fs_name} = new FsNull;
        ...
        $class_name = implode('_', $words);
        if ( class_exists( $class_name ) ) {
            //echo $class_name . "<br>";
            $class_name::instance()->init();
        }
    }
}

if ( 0 == did_action( 'owp_fs_loaded' ) ) {
    add_action( 'owp_fs_loaded', 'oceanwp_addons_fs', 15 );
} else {
